...
class FG_eval {
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& z) {
    ...
    assert(fg.size() - 1 == (m_N)*nx + (m_N + 1) * mx + m_N * mu &&
           "we don't have enough constraints!");

    for (int t = 0; t < m_N; t++) {
      // unfortunatley ADvectors don't seem to have any of the standard iterator
      // objects, so need to manually extract subvectors
...
VectorXd MPC::Solve(const VectorXd& x0, int N, const MatrixXd& A,
  ...
  // Set the number of model variables (includes both states and inputs). nVars
  // = nx * (N+1) + nu * (N)
  ...
  size_t nConstraints =
      nDynamicsConstraints + nStateConstraints + nInputConstraints;
  ...
  // No upper lower bounds on variables, we'll enforce those using polytopic
  // constraints Fx*x(t) <= bx, Fu*u(t) <= bu
  ...
  // TODO: need to handle infeasibiliyt more gracefully, right now we just break
  // -- could also shift state/input constraints to soft constraints for now,
  // just kill things
  assert(ok && "MPC solve failed");

  // Build a VectorXd out of the first control input and return that to be
