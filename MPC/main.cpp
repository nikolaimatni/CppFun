...
This is more just a project for me to play around with c++11 features than
anything else
 */
...
int main() {
  ...
  // Let's test our new System implementation
  unique_ptr<System> lti_sys(new LtiSystem(A, B, H, Q, R));

  VectorXd x0 = VectorXd(3);
  x0 << 1, 2, 3;

  // Set up an LTI simulator and a MPC simulator using the same dynamics
  ...
  MySimulator my_sim(5, x0, lti_sys.get());

  my_sim.Simulate();
  my_sim.WriteToFile("lti_xlog.tr", "lti_ulog.tr");
  ...
  // give same control action as Kx0
